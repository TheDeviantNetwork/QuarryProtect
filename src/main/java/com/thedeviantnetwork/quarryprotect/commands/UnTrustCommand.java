...
public class UnTrustCommand implements CommandExecutor {
    ...
    public UnTrustCommand(QuarryRegionManager regionManager){
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (strings.length == 1){
            ...
            if(!target.isPresent()){
                Util.sendMessage(commandSender, ChatColor.RED + "Player " + strings[0] + " can't be found");
            }else if (!region.isPresent()){
                Util.sendMessage(commandSender, ChatColor.RED + "You are not standing in the protected area!");
            }else {
                region.get().delPlayer(target.get());
